    ...
    if (test_name) {
      switch (test_name.toLowerCase()) {
        case "labeling":
          ...
          const Recommendation = response_data;

          const text = txt_input.value.trim();
          const ids = personal_ids.value;
            ...
            fetch("/test-srv", {
              ...
              .then((data) => {
                ...
                for (idx = 0; idx < dictObj.texts.length; idx++) {
                  ...
                  htmlText =
                    '<span style="color:#' + dNh(c, 6) + '">' + t + "<span><br>";
        ...
        default:
          ...
          fetch("/test-srv", {
            ...
            body: JSON.stringify({
              selected_test: test_name, //define all params
            ...
            .then((data) => {
              // Update the Recommendation section with the server's response
              //console.log(data);
              response_data.innerHTML = data.text || "No suggestions received."; // Check if data.suggestions is defined
            })
            .catch((error) => {
              console.error("Error:", error);
              //Recommendation.textContent = "An error occurred: " + error.message; // Display a more detailed error message
